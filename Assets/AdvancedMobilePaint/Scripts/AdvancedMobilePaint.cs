	...
	public class AdvancedMobilePaint : MonoBehaviour
	{
		...
		void MousePaint()
		{
			...
			if (Input.GetMouseButton(0) && Input.touchCount <= 1)
			{
				...
				if (!Physics.Raycast(useAlternativeRay ? Camera.main.ScreenPointToRay(Camera.main.WorldToScreenPoint(raySource.position)) : Camera.main.ScreenPointToRay(Camera.main.WorldToScreenPoint(drawPoint.position)), out hit, Mathf.Infinity, paintLayerMask))
				{
				...
				if (tx * tx + ty * ty < r2)//(if in circle) 
				{
					...
					if (useAdditiveColors)
					{
						...
						if (!useLockArea || (useLockArea && lockMaskPixels[pixel] == 1))
						{
							...
							/*float*/
							yy = Mathf.Repeat(y + ty, customPatternWidth);
							/*float*/
							xx = Mathf.Repeat(x + tx, customPatternWidth);
							/*int*/
							pixel2 = (int)Mathf.Repeat((customPatternWidth * xx + yy) * 4, patternBrushBytes.Length);
					...
					{ // no additive, just paint my colors
						...
						if (!useLockArea || (useLockArea && lockMaskPixels[pixel] == 1))
						{
							...
							yy = Mathf.Repeat(y + ty, customPatternWidth);
							/*float*/
							xx = Mathf.Repeat(x + tx, customPatternWidth);//Debug.Log ("P"+xx+","+yy);
							/*int*/
							pixel2 = (int)Mathf.Repeat((customPatternWidth * xx + yy) * 4, patternBrushBytes.Length);
		...
		//박진범 추가_230502
		public void ReadCurrentCustomPattern2(Texture2D patternTexture)
		{
			if (patternTexture == null) {/*Debug.LogError("Problem: No custom patterns assigned on "+gameObject.name);*/ return; }
			this.pattenTexture = patternTexture;
			customPatternWidth = patternTexture.width;
			...
			for (int x = 0; x < customPatternWidth; x++)
			{
				for (int y = 0; y < customPatternHeight; y++)
				{
					Color brushPixel = tmp[x * customPatternHeight + y];//patternTexture.GetPixel(x,y);
					...
					//END FIX
					{
						...
						if (useCustomBrushAlpha)
						{
							if (useAdditiveColors)
							{
								...
								{//this enables custom brushes using lock mask
									...
									switch (brushMode)
									{
										...
										case BrushProperties.Pattern:
											//TODO
											/*float*/
											yy = Mathf.Repeat(/*py+y*/py + (y - customBrushHeight / 2f), customPatternHeight);
											/*float*/
											xx = Mathf.Repeat(/*px+x*/px + (x - customBrushWidth / 2f), customPatternWidth);
											/*int*/
											pixel2 = (int)Mathf.Repeat((customPatternWidth * xx + yy) * 4, patternBrushBytes.Length);
